...
class HTTPCookieArray implements IteratorAggregate, ArrayAccess, Countable {
	...
	protected function getArray() {
		...
		if ($this->message instanceof HTTPRequest)
			parse_str(str_replace(';', '&', $this->message->getHeader('Cookie')), $cookies);		
		// parse HTTPResponse Set-Cookie: header
		if ($this->message instanceof HTTPResponse)
			parse_str(implode('&', preg_replace('/\s*;.*$/', '',
			    (array) $this->message->getHeader('Set-Cookie'))), $cookies);
	...
	static public function getCurrent() {
		...
		$request->setURIComponents(array(
			...
			'query' => strpos($_SERVER['REQUEST_URI'], '?') ?
			    preg_replace('/^.*?\?$/', '', $_SERVER['REQUEST_URI']) : null,
...
#[TODO] expand this?
	...
	public function getParsedQueryData() {
		// parse the query
		parse_str($this->getURIComponents()->query, $data);
		return $data;
	}
...
class MIMEType extends HTTPType {
	...
	function __construct($type, $subtype, $params = array()) {
